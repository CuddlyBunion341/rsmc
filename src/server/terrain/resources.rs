...
pub struct ClientChunkRequests {
    queues: HashMap<ClientId, VecDeque<IVec2>>,
}

impl ClientChunkRequests {
    pub fn enqueue_bulk(&mut self, client_id: ClientId, chunk_positions: &mut VecDeque<IVec2>) {
        self.queues
            .entry(client_id)
            .or_default()
    ...
    where
        F: FnMut(&ClientId, &mut VecDeque<IVec2>) -> bool,
    {
